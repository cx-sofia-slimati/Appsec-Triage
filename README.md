...
This application uses a hardened Docker configuration with:
- **Pinned package versions** in Alpine Linux for reproducible and secure builds
- **Non-root user** execution for container security
- **Health checks** for monitoring application status

### Package Version Management

Package versions are pinned in the Dockerfile to prevent supply chain attacks and ensure reproducible builds. See `.package-versions` file for current versions and update instructions.

To update package versions:
1. Run `./update-packages.sh` to check for newer versions
2. Verify no CVEs exist for the new versions
3. Test the build and functionality
4. Update both Dockerfile and `.package-versions` file
